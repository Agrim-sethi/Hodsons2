...
for (let i = 1; i < lines.length; i++) {
    ...
    // SN,Comp No,Name,Grn,House,Class,DOB,Age as on 15 April
    const id = cols[1];
    const name = cols[2];
    const gender = cols[3];
    const houseStr = cols[4];
    const dobStr = cols[6]; // e.g. 12/23/2016 (MM/DD/YYYY or DD/MM/YYYY)
    let ageStr = cols[7] || '';
    ...
    if (ageStr.includes('Y')) {
        ...
        // Calculate age from DOB if ageStr is empty
        // Assuming DOB could be either DD/MM/YYYY or MM/DD/YYYY. Usually in India it's DD/MM/YYYY.
        ...
        let target = new Date(2026, 3, 15); // 15 April 2026
    ...
    // Edge Case rule checking: if somehow BD gets Under 12? Unlikely based on age division expectations, 
    // but we will assign strictly by age and if category is missing from predefined list we map it up.
    // e.g., BD Under 11 -> BD Under 13!
    ...
    if (!validCategories.includes(finalCategory)) {
        // Coerce missing boundary cases correctly!
        if (dept === 'BD' || dept === 'GD') {
            if (ageYears <= 12) finalCategory = `${dept} Under 13`; // BD Under 11/12 goes to BD U13
        }
    }
