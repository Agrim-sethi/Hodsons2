  ...
  return (
    <section className="space-y-7 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div><div className="royal-kicker mb-1">Championship Ledger</div><h2 className="text-3xl font-black tracking-tight text-white">Athletics 2026 Summary</h2><p className="mt-1 max-w-4xl text-sm leading-relaxed text-slate-400">A complete podium ledger for every eligible event in every Athletics age category. Finals are shown when allotted, otherwise the qualifying results form the event podium.</p></div>
        <div className="flex flex-wrap items-center gap-2"><button disabled={Boolean(downloading)} onClick={downloadXlsx} className="flex items-center gap-2 rounded-xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-2.5 text-xs font-black uppercase tracking-[0.16em] text-emerald-300 hover:bg-emerald-500/15 disabled:opacity-50"><Icon name={downloading === 'xlsx' ? 'sync' : 'table_chart'} className={downloading === 'xlsx' ? 'animate-spin' : ''} size="16" /> .xlsx</button><button disabled={Boolean(downloading)} onClick={downloadDocx} className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/10 px-4 py-2.5 text-xs font-black uppercase tracking-[0.16em] text-primary hover:bg-primary/15 disabled:opacity-50"><Icon name={downloading === 'docx' ? 'sync' : 'description'} className={downloading === 'docx' ? 'animate-spin' : ''} size="16" /> .docx</button></div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><div className="rounded-2xl border border-primary/10 bg-primary/[0.04] px-4 py-3"><div className="text-[9px] font-black uppercase tracking-[0.2em] text-primary/70">Categories</div><div className="mt-1 text-2xl font-black text-white">{summaries.length}</div></div><div className="rounded-2xl border border-white/8 bg-white/[0.025] px-4 py-3"><div className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Eligible Events</div><div className="mt-1 text-2xl font-black text-white">{summaries.reduce((sum, category) => sum + category.events.length, 0)}</div></div><div className="rounded-2xl border border-white/8 bg-white/[0.025] px-4 py-3"><div className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Podiums Recorded</div><div className="mt-1 text-2xl font-black text-primary">{podiumCount}</div></div><div className="rounded-2xl border border-white/8 bg-white/[0.025] px-4 py-3"><div className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Places Per Event</div><div className="mt-1 text-2xl font-black text-white">1st • 2nd • 3rd</div></div></div>

      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-primary/10 bg-black/15 px-4 py-3"><span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Podium Key</span>{['1st', '2nd', '3rd'].map((label,index)=><span key={label} className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-wider ${podiumRankStyle(index)}`}><Icon name="emoji_events" size="12" /> {label}</span>)}<span className="ml-auto text-[10px] text-slate-500">TBD means no finished result has been recorded yet.</span></div>
      ...
      {summaries.map(category => (
        <section key={category.category} className="glass-panel overflow-hidden rounded-[30px] border border-primary/12">
          <div className="flex flex-col gap-3 border-b border-primary/10 bg-[linear-gradient(135deg,rgba(201,163,74,0.10),rgba(255,255,255,0.015))] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><div className="royal-kicker mb-1">Age Category Podiums</div><h3 className="text-2xl font-black tracking-tight text-white">{category.category}</h3></div><div className="rounded-xl border border-white/8 bg-black/15 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{category.events.length} eligible events</div></div>
          <div className="overflow-x-auto custom-scrollbar"><table className="w-full min-w-[980px] border-collapse"><thead><tr className="bg-white/[0.025] text-[9px] font-black uppercase tracking-[0.22em] text-slate-500"><th className="w-[24%] px-5 py-4 text-left">Event</th><th className="w-[25%] px-4 py-4 text-left">1st Place</th><th className="w-[25%] px-4 py-4 text-left">2nd Place</th><th className="w-[26%] px-4 py-4 text-left">3rd Place</th></tr></thead><tbody className="divide-y divide-white/5">
